...
/** 无 sso key 时的默认限流/配额(与 ensureUserKey / ApiKey 默认值一致) */
const DEFAULT_RATE_LIMIT = 60;
const DEFAULT_DAILY_QUOTA = 100000;
const DEFAULT_MONTHLY_QUOTA = 3000000;
...
/// tokens = tokensIn + tokensOut(与 dashboard bucket() 一致); cost 为聚合和
...
export async function meRoutes(fastify: FastifyInstance) {
  fastify.get('/api/me/usage', { preHandler: [fastify.authenticateSso] }, async (req): Promise<UsageSummary> => {
    ...
    if (ssoKey) {
      // 总量口径与 internal.ts verify 一致: 按该 key 的 usageRecord 聚合
